...
class NanoBananaApp {
    ...
    setupEventListeners() {
        ...
        document.querySelectorAll('.ratio-btn').forEach(button => {
            button.addEventListener('click', (e) => {
                this.selectRatio(e.currentTarget.dataset.ratio);
            });
        });
        ...
        document.querySelectorAll('.style-btn').forEach(button => {
            button.addEventListener('click', (e) => {
                this.selectStyle(e.currentTarget.dataset.style);
            });
        });
    ...
    selectRatio(ratio) {
        ...
        // Update old button style (if exists)
        ...
        // Keep pill display text as "Ratio" - don't change it
        // The selected value is shown in the dropdown options
    }
    ...
    selectStyle(style) {
        ...
        // Keep pill display text as "Style" - don't change it
        // The selected value is shown in the dropdown options
    }
...
// App will be initialized from index.html
...
/
/ Enhanced functionality for the optimized landing page
class LandingPageEnhancements {
    ...
    init() {
        this.setupUseCaseTabs();
        this.setupExampleTags();
        this.setupScrollAnimations();
        this.setupModeTabsFixed();
        this.setupDropdowns();
    ...
    setupModeTabsFixed() {
        ...
        tabs.forEach(tab => {
            tab.addEventListener('click', () => {
                ...
                const targetContent = document.getElementById(mode === 'text-to-image' ? 'textToImageMode' : 'imageToImageMode');
                if (targetContent) {
        ...
        pills.forEach(pill => {
            ...
            options.forEach(option => {
                option.addEventListener('click', (e) => {
                    ...
                    // Remove active from all options in this pill
                    options.forEach(opt => opt.classList.remove('active'));
                    
                    // Add active to selected option
                    option.classList.add('active');
                    
                    // Update display value and sync with main app
                    const ratio = option.dataset.ratio;
                    const style = option.dataset.style;
                    
                    if (ratio) {
                        pillText.textContent = ratio;
                        // Update both text elements if they exist
                        const ratioTexts = document.querySelectorAll('#ratioText, #ratioTextI2I');
                        ratioTexts.forEach(text => text.textContent = ratio);
                        
                        // Sync with main app
                        if (window.nanoBananaApp) {
                            window.nanoBananaApp.selectRatio(ratio);
                        }
                    } else if (style) {
                        const styleName = style.charAt(0).toUpperCase() + style.slice(1);
                        pillText.textContent = styleName;
                        // Update both text elements if they exist
                        const styleTexts = document.querySelectorAll('#styleText, #styleTextI2I');
                        styleTexts.forEach(text => text.textContent = styleName);
                        
                        // Sync with main app
                        if (window.nanoBananaApp) {
                            window.nanoBananaApp.selectStyle(style);
                        }
                    }
    ...
    setupClearButtons() {
        // Clear button for text-to-image prompt
        ...
        if (clearPromptBtn && promptInput) {
            clearPromptBtn.addEventListener('click', () => {
                ...
                // Add visual feedback
                clearPromptBtn.style.transform = 'scale(0.9)';
                setTimeout(() => {
                    clearPromptBtn.style.transform = '';
                }, 150);
            });
        }
        
        // Clear button for image-to-image prompt
        ...
        if (clearTransformBtn && transformPrompt) {
            clearTransformBtn.addEventListener('click', () => {
                transformPrompt.value = '';
                transformPrompt.focus();
                
                // Add visual feedback
                clearTransformBtn.style.transform = 'scale(0.9)';
                setTimeout(() => {
                    clearTransformBtn.style.transform = '';
                }, 150);
            });
        }
    ...
    handleImageFile(file) {
        ...
        if (!validTypes.includes(file.type)) {
            this.showUploadError('Please upload a JPG, PNG, or WebP image file.');
            return;
        }
    ...
    setupUseCaseTabs() {
        ...
        tabs.forEach(tab => {
            tab.addEventListener('click', () => {
                ...
                // Show corresponding content
                const caseType = tab.dataset.case;
                const targetContent = document.getElementById(`${caseType}Case`);
                if (targetContent) {
                    targetContent.classList.add('active');
                }
            });
    ...
    setupScrollAnimations() {
        ...
        const animateElements = document.querySelectorAll(
            '.testimonial-card, .problem-item, .solution-item, .use-case-example'
        );
...
// Initialize landing page enhancements when DOM is loaded
document.addEventListener('DOMContentLoaded', () => {
    new LandingPageEnhancements();
});

// Enhanced image generation with better user feedback
class EnhancedImageGenerator {
    constructor() {
        this.setupEnhancedGeneration();
    }
    
    setupEnhancedGeneration() {
        const generateBtn = document.getElementById('generateBtn');
        if (generateBtn) {
            generateBtn.addEventListener('click', this.handleEnhancedGeneration.bind(this));
        }
    }
    
    async handleEnhancedGeneration() {
        const promptInput = document.getElementById('promptInput');
        const transformPrompt = document.getElementById('transformPrompt');
        const currentMode = document.querySelector('.tab-button.active')?.dataset.mode || 'text-to-image';
        
        let prompt = '';
        if (currentMode === 'text-to-image') {
            prompt = promptInput?.value.trim();
        } else {
            prompt = transformPrompt?.value.trim();
        }
        ...
        if (imageDisplaySection) {
            imageDisplaySection.classList.add('active');
            imageDisplaySection.scrollIntoView({ behavior: 'smooth' });
        }
        
        // Use existing generation logic from the main app
        if (window.nanoBananaApp) {
            window.nanoBananaApp.generateImage();
        }
    }
    
    showError(message) {
        // Create or update error message
        let errorDiv = document.querySelector('.generation-error');
        if (!errorDiv) {
            errorDiv = document.createElement('div');
